...
const firebaseConfig = {
  ...
  authDomain: "mes-pochettes.firebaseapp.com",
  projectId: "mes-pochettes",
  storageBucket: "mes-pochettes.firebasestorage.app",
  messagingSenderId: "81130015768",
  appId: "1:81130015768:web:b3771542439117197f876d"
};

const firebaseApp = initializeApp(firebaseConfig);
const db = getFirestore(firebaseApp);
const USER_ID = "siana447";
...
export default function App() {
  const [pochettes, setPochettes] = useState(DEFAULT_POCHETTES);
  ...
  function updatePochettes(newP) { setPochettes(newP); save(newP, budget); }
  function updateBudget(newB) { setBudget(newB); save(pochettes, newB); }

  const totalMensuel = pochettes.reduce((s, p) => s + p.mensuel, 0);
  const reste = budget - totalMensuel;

  function addPochette(data) { updatePochettes([...pochettes, { ...data, id: Date.now() }]); setModal(null); }
  function updatePochette(data) { updatePochettes(pochettes.map(x => x.id === editing.id ? { ...x, ...data } : x)); setModal(null); setEditing(null); }
  function deletePochette(id) { updatePochettes(pochettes.filter(x => x.id !== id)); setModal(null); setEditing(null); }
  function doDeposit() {
    const m = +depositForm.montant;
    if (!m || m <= 0) return;
    updatePochettes(pochettes.map(x => x.id === depositForm.pochetteId ? { ...x, epargne: x.epargne + m } : x));
  ...
  if (loading) {
    return (
      <div style={{ display: "flex", alignItems: "center", justifyContent: "center", height: "100vh", fontFamily: "'DM Sans', sans-serif", color: "#64748b", fontSize: 15 }}>
        <div style={{ textAlign: "center" }}>
          <div style={{ fontSize: 40, marginBottom: 12 }}>💰</div>
          Chargement de tes pochettes...
        </div>
      </div>
    );
  }
      ...
      <div style={{ padding: "1.25rem", maxWidth: 680, margin: "0 auto" }}>
        ...
        {pochettes.length > 0 && (
          <div style={{ background: "#fff", borderRadius: 14, padding: "1rem 1.25rem", border: "1px solid #e2e8f0", marginBottom: "1.5rem" }}>
            ...
            <div style={{ display: "flex", height: 14, borderRadius: 99, overflow: "hidden", gap: 2, marginBottom: 10 }}>
              {pochettes.map(p => (<div key={p.id} style={{ flex: p.mensuel, background: COLORS[p.color].bg }} title={p.name} />))}
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(280px, 1fr))", gap: 12, marginBottom: "1.5rem" }}>
          {pochettes.map(p => {
            ...
            return (
              <div key={p.id} style={{ background: "#fff", borderRadius: 14, border: "1px solid #e2e8f0", padding: "1rem 1.25rem" }}>
          ...
          <div onClick={() => setModal("add")}
            style={{ background: "#f8fafc", borderRadius: 14, border: "1.5px dashed #cbd5e1", padding: "1rem", display: "flex", alignItems: "center", justifyContent: "center", flexDirection: "column", gap: 8, cursor: "pointer", minHeight: 160, color: "#94a3b8" }}>
        ...
        {reste < 0 && (
          <div style={{ background: "#fff5f5", border: "1px solid #fecaca", borderRadius: 12, padding: "12px 16px", fontSize: 13, color: "#dc2626" }}>
            ⚠️ Tes versements dépassent ton budget de {formatEur(Math.abs(reste))}.
          </div>
        )}
        {reste > 0 && pochettes.length > 0 && (
          <div style={{ background: "#f0fdf4", border: "1px solid #bbf7d0", borderRadius: 12, padding: "12px 16px", fontSize: 13, color: "#16a34a" }}>
            💡 Il te reste {formatEur(reste)} libres chaque mois !
          </div>
        )}
      </div>

      {modal === "add" && <Modal onClose={() => setModal(null)}><PochetteForm onSave={addPochette} onCancel={() => setModal(null)} /></Modal>}
      {modal === "edit" && editing && (
        <Modal onClose={() => { setModal(null); setEditing(null); }}>
          ...
          <button onClick={() => deletePochette(editing.id)} style={{ width: "100%", marginTop: 8, padding: 10, background: "#fff5f5", color: "#dc2626", border: "1px solid #fecaca", borderRadius: 10, fontSize: 13, cursor: "pointer" }}>
            🗑️ Supprimer cette pochette
          </button>
        </Modal>
      )}
      {modal === "budget" && <Modal onClose={() => setModal(null)}><BudgetSetup budget={budget} onSave={v => { updateBudget(v); setModal(null); }} onCancel={() => setModal(null)} /></Modal>}
      {modal === "deposit" && (
